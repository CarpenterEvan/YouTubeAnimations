...
for i, sigma_i in enumerate(parameter_space):
	full_sol = solve_ivp(lorenz, t_span = t_sp, y0 = r0, t_eval = t_ev, args=(sigma_i, rho, beta))
	x, y, z = full_sol.y
	t = full_sol.t
...
x_ax_limit, y_ax_limit, z_ax_limit = get_ranges(x_data, y_data, z_data, classic=True)
...
yz_line = Line2D(y_data, z_data, axes=yz_ax, linewidth=1., color="blue", zorder=1, rasterized=True, animated=False)
xz_line = Line2D(x_data, z_data, axes=xz_ax, linewidth=1., color="blue", zorder=1, rasterized=True, animated=False)
yx_line = Line2D(y_data, x_data, axes=yx_ax, linewidth=1., color="blue", zorder=1, rasterized=True, animated=False)

x_line = Line2D(t_data, x_data, linewidth=1., color="cyan", zorder=1, label="x", rasterized=True, animated=False)
y_line = Line2D(t_data, y_data, linewidth=1., color="red", zorder=1, label="y", rasterized=True, animated=False)
z_line = Line2D(t_data, z_data, linewidth=1., color="green", zorder=1, label="z", rasterized=True, animated=False)
...
def update(data):
	...
	for point in unique_fixed_point_magnitudes:
		ax4_fixed_points_line = ax4.axhline(point, linewidth=0.5, color="white", zorder=0, linestyle=":")
