...
y_line = Line2D([t[0]], [y[0]], linewidth=1., color="green",
                zorder=1, label="y", rasterized=True)
z_line = Line2D([t[0]], [z[0]], linewidth=1., color="cyan",
                zorder=1, label="z", rasterized=True)
...
def make_frame(i):
    ...
    plt.savefig(
        f"{sys.path[0]}/butterfly_frames/_img{int(t[i]//di):04d}.png",
        dpi=20)
    ...
    #print(f"", end="\r")
    print(
        f"\033[{move}A\u001b[0K{percent_per_core} -- {time_frame_took}",
        end="\r")
