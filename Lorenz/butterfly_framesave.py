...
yz_fixed_point_1 = plot_circle(
    (fixed_point_a[1], fixed_point_a[2]), fixed_point=True)
xz_fixed_point_1 = plot_circle(
    (fixed_point_a[0], fixed_point_a[2]), fixed_point=True)
yx_fixed_point_1 = plot_circle(
    (fixed_point_a[1], fixed_point_a[1]), fixed_point=True)

yz_fixed_point_2 = plot_circle(
    (fixed_point_b[1], fixed_point_b[2]), fixed_point=True)
xz_fixed_point_2 = plot_circle(
    (fixed_point_b[0], fixed_point_b[2]), fixed_point=True)
yx_fixed_point_2 = plot_circle(
    (fixed_point_b[1], fixed_point_b[1]), fixed_point=True)
...
x_line = Line2D([t_ev[0]], [r0[1]], linewidth=1., color="red",
                zorder=1, label="x", vectorized=True)
y_line = Line2D([t_ev[0]],
                [r0[2]],
                linewidth=1.,
                color="green",
                zorder=1,
                label="y",
                vectorized=True)
z_line = Line2D([t_ev[0]], [r0[2]], linewidth=1., color="cyan",
                zorder=1, label="z", vectorized=True)
...
initial_condition_string = r"\noindent$x_0=${:.4f} $y_0=${:.4f} $z_0=${:.4f}".format(
    r0[0], r0[1], r0[2])
parameters_string = r"$\sigma=${:.4f} $\rho=${:.4f} $\beta=${:.4f}".format(
    sigma, rho, beta)
fig.suptitle(
...
ax4.axhline(-np.sqrt(beta * (rho - 1)),
            linewidth=0.5,
            color="white",
            zorder=0,
            linestyle=":")  # plot horizontal dotted line for each fixed point
