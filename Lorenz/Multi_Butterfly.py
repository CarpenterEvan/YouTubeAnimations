...
x_data = np.zeros(shape)#*r[0][0] Can i just put in the x solution as the data right now?
...
for i in range(number):
	...
	yz_circ[i] = plt.Circle((y_data[i][0], z_data[i][0]), .5, color=color_list[i])

	yz_line[i] = Line2D([y_data[i][0]], [z_data[i][0]], linewidth=1., color=color_list[i], zorder=1)
...
yx_line[i] = Line2D(y_data[i], x_data[i], linewidth=1., color=color_list[i], zorder=1)
...
for i in range(number):
	...
	#xz_line[i] = Line2D(z_data[i], x_data[i], linewidth=1., color=color_list[i], zorder=1)
	yz_line[i] = Line2D(y_data[i], z_data[i], linewidth=1., color=color_list[i], zorder=1)
	xz_line[i] = Line2D(x_data[i], z_data[i], linewidth=1., color=color_list[i], zorder=1)
	yx_line[i] = Line2D(y_data[i], x_data[i], linewidth=1., color=color_list[i], zorder=1)
...
def update(input_data_list):
	for i in range(number):
		yz_line[i], xz_line[i], yx_line[i], diff_line[i], yz_circ[i], yx_circ[i], xz_circ[i] = update(input_data_list[i])
...
ani = FuncAnimation(fig, update, zip(x1, y1, z1, t), interval=10, blit=True, repeat=False, save_count=len(t_ev))
