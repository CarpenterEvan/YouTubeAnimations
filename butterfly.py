...
yz_line = Line2D(y_data, z_data, linewidth=1., color="blue", zorder=1)
xz_line = Line2D(z_data, x_data, linewidth=1., color="blue", zorder=1)
yx_line = Line2D(y_data, x_data, linewidth=1., color="blue", zorder=1)

x_line = Line2D(t_data, x_data, linewidth=1., color="cyan", zorder=1, label="x")
y_line = Line2D(t_data, y_data, linewidth=1., color="red", zorder=1, label="y")
z_line = Line2D(t_data, z_data, linewidth=1., color="green", zorder=1, label="z")
...
for point in unique_fixed_point_magnitudes:
	ax4.add_line(Line2D(t_ev, [point]*len(t_ev), linewidth=0.5, color="magenta", zorder=0, linestyle=":"))

for point in fixed_points:

	x_point, y_point, z_point = point
	yz_ax.add_patch(plt.Circle((y_point, z_point), .05, color="white"))
	xz_ax.add_patch(plt.Circle((x_point, z_point), .05, color="white"))
	yx_ax.add_patch(plt.Circle((y_point, x_point), .05, color="white"))
...
ax4.set(title="Magnitude of each component",
		...
		xlim=[0,40], ylim=[smallest_value, biggest_value])
...
ani = FuncAnimation(fig, update, zip(x, y, z, t), interval=17, blit=True, repeat=False, save_count=len(t_ev))
...
if save =="y":
	...
	ani.save(f"chaos_s{sigma:.0f}b{beta:.0f}r{rho:.0f}_x{r0[0]:.0f}y{r0[1]:.0f}z{r0[2]:.0f}.mp4", writer=writervideo, dpi=240)
	print("done!")