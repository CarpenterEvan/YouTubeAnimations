...
x_line = Line2D(t_data, x_data, linewidth=1., color="cyan", zorder=1, label="x", rasterized=True, animated=True)
y_line = Line2D(t_data, y_data, linewidth=1., color="red", zorder=1, label="y", rasterized=True, animated=True)
z_line = Line2D(t_data, z_data, linewidth=1., color="green", zorder=1, label="z", rasterized=True, animated=True)
...
def make_frame(i):
	...
	plt.savefig(f'ChaosAnimations/butterfly_frames/_img{i//di:04d}.png', dpi=100)
	yz_ax.cla()
